/* ---
   script.js
   Handles component loading, hybrid navigation with internal close, scroll behaviors, animations, and audio.
   --- */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeNavbar() {
        ...
        const panelCloseBtn = document.getElementById('panel-close-btn'); // Get the new close button

        if (navToggle && slideInPanel && pageOverlay && panelCloseBtn) {
            const toggleMenu = () => {
                // The hamburger icon itself no longer needs a state class
            ...
            // Event listeners
            navToggle.addEventListener('click', toggleMenu);     // Open with hamburger
            pageOverlay.addEventListener('click', toggleMenu);   // Close with overlay
            panelCloseBtn.addEventListener('click', toggleMenu); // Close with the 'X' button
        }
    }
    ...
    /**
     * Fetches the navbar component and injects it into the placeholder.
     */
    function loadNavbar() {
        fetch('components/navbar.html')
            ...
            .catch(error => {
                ...
                document.getElementById('navbar-placeholder').innerHTML = '<p style="text-align:center; padding: 1rem;">Error: Navigation could not be loaded.</p>';
            });
    }
    ...
    // --- Scroll-Triggered Animation Logic for page sections ---
