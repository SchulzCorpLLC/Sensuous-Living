...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Fetches a component, rewrites its internal links to be relative to the current page,
     * and injects it into its placeholder.
    ...
    function loadComponent(componentName, placeholderId, callback) {
        fetch(`${basePath}components/${componentName}`)
            ...
            .then(html => {
                ...
                // Find all anchor tags and rewrite their href attributes
                const links = tempContainer.querySelectorAll('a');
                links.forEach(link => {
                    const href = link.getAttribute('href');
                    // Only rewrite relative links, ignore absolute URLs and anchor links
